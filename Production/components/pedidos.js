let pedidos = {
        ...
        listaPedidos() {
            ...
            if (this.$props.listaPedidos != null) {
                if (this.$props.listaPedidos[0].metodo == undefined) {
                    ...
                    this.$props.listaPedidos.forEach(pedido => {
                        ///agregado 2 ineas
                        const productos = Array.isArray(pedido.Bodegas)
                        ? JSON.parse(JSON.stringify(pedido.Bodegas)).reduce((acc, el) => acc.concat(Array.isArray(el.Productos) ? el.Productos : []), [])
                         : [];
                        ...
                        const objPedido = {
                            ...
                            nombreEntrega: productos[0].nombreEntrega ,
                            numeroReferencia: productos[0].numeroReferencia,
                            activacionSeguimiento: productos[0].activacionSeguimiento ,
                            productos: productos
                        }
                        // console.log("pedido.Bodegas:", pedido.Bodegas);
                        /*
                        const objPedido = {
                            idPedido: pedido.idPedido,
                            fechaHoraPedido: pedido.fechaHoraPedido,
                            ...
                            direccion: pedido.direccion,
                            detalleDireccion: pedido.detalleDireccion,
                            url: productos.length > 0 ? productos[0].url || '' : '',
                            fechaEntrega: productos.length > 0 ? productos[0].fechaEntrega || '' : '',
                            nombreEntrega: productos.length > 0 ? productos[0].nombreEntrega || '' : '',
                            numeroReferencia: productos.length > 0 ? productos[0].numeroReferencia || '' : '',
                            activacionSeguimiento: productos.length > 0 ? productos[0].activacionSeguimiento || '' : '',
                            productos: productos
                        };
                         */
                        numm++;
    ...
    methods: {
        ...
        ejecutascript(){
            alert("se ejecuto el script");
        },
        cancelarPedido(pedido){
            createQuestion("Cancelación de Pedido", "Está a punto de cancelar el pedido #" + pedido.idPedido + " ¿Está seguro?",true,()=>{
                $.ajax({
                    type: 'POST',
                    url: 'https://arvispace.com/serviciosASAR/pedidoCancelado.php',//entregarPedidoAR.php
                    data: 'idPedido=' + pedido.idPedido,
                    
                    success: function (response) {

                        console.log("respuesta despacho" + response);

                        if (response == "1") { //A este punto, el pedido ha cambiado a status 4 (Cancelado)
                            //Devolucion - reembolso    
                            $.ajax({
                                url:'https://arvispace.com/lib/payment/devolucion.php',
                                type:'POST',
                                data:'idBanco=' + pedido.idBanco,
                               
                                success:function (responseRefund){
                                    console.log(responseRefund);
                                    // json= JSON.parse(responseRefund);

                                    if (responseRefund =="completed")// La devolución
                                    {
                                        createAviso("Su pedido #" + pedido.idPedido + " fue cancelado y será reembolsado a la brevedad. Actualizando lista de pedidos...");
                                        setTimeout(function(){
                                ...
                                error:function(){
                                    console.error('Error en la solicitud de Devolucion');
                                    createAviso("Su pedido #" + pedido.idPedido + "fue cancelado, pero hubo un error en la solicitud de devolución");
                                }
                            });
                                                     
                        }
                        else {
                            createAviso("Hubo un error al cancelar su pedido, espere unos minutos y pruebe nuevamente.");
                        }
            ...
            <div class="container">
                <div class="nav-wrapper">
                    <ul role="tablist" class="nav nav-pills nav-fill">
                        ...
                        <li class="nav-item" @click="verCancelados">
                            <a data-toggle="tab" role="tab" href="#undefined" class="nav-link" :class="statusSelected == 4 ? 'active':''">
                                <div>
                                    <i class="icon-Cancelar mr-1"></i>
                                    Cancelado
                                </div>
                            </a>
                        </li>
                ...
                <div v-if="pedidos.length" class="row">
                    <div class="card mt-2 mb-2" v-if="pedido.estatus==statusSelected" v-for="pedido in pedidos">
                        <div class="card-body">
                            ...
                            <p v-if="validateView(['2','3'])"><b>Fecha de envío del pedido:</b> {{pedido.fechaEntrega}}</p>
                            <p v-if="validateView(['2','3'])"><b>Paqueteria:</b> {{pedido.nombreEntrega}}</p>
                            <p v-if="validateView(['2','3'])"><b>Número de rastreo:</b> {{pedido.numeroReferencia}}</p>
                            <p v-if="validateView(['1','2','3','4'])"><b>Dirección de entrega:</b> {{pedido.direccion}}</p>
                            <p v-if="validateView(['1','2','3','4'])"><b>Total:</b> {{pedido.total | toCurrency}}</p>
                            <a class="" data-toggle="collapse" :href="'#productos-'+pedido.idPedido" role="button" aria-expanded="false">
                                Ver Productos
                            </a>
                            <div class="modal-footer" v-if="validateView(['1'])" @click="cancelarPedido(pedido)"><button type="submit" class="btn btn-secondary" style="width: 100%; filter: invert(0%);">Cancelar Pedido</button></div>
                            <div class="modal-footer" v-if="validateView(['2'])" @click="rastreo(pedido.url)"><button type="submit" class="btn btn-secondary" style="width: 100%; filter: invert(0%);">Rastrear</button></div>

                            <div v-if="producto.idPedido==pedido.idPedido" v-for="producto in productos" class="collapse" :id="'productos-'+pedido.idPedido">
                            
                                
                                 <div class="card-body" style="display: flex; align-items: center; justify-content: space-between; flex-wrap: nowrap; gap: 16px;">
                                    ...
                                    ></carousel>
                                    <p v-if="validateView(['1','2','3','4'])"><b>Cantidad de productos:</b> {{producto.cantidad}}</p>
                                    <p v-if="validateView(['1','2','3','4'])"><b>Total:</b> {{producto.total | toCurrency}}</p>
                            ...
                            </div><!-- FIN div v-if="producto.idPedido==pedido.idPedido" v-for="producto in productos" class="collapse" :id="'productos-'+pedido.idPedido"-->
                        </div><!-- FIN div class="card-body"-->

                        <!---->


                    </div><!-- FIN div class="card mt-2 mb-2" v-if="pedido.estatus==statusSelected" v-for="pedido in pedidos"-->
                
                    <div>
                        <div class="anim__container container" v-if="statusSelected==1 && numstatus1==0">
                            <!--<div class="background" style="height:auto;width: 100.5vw;position: absolute;margin-left: -14.5vw;object-fit: inherit; ">-->
                            ...
                            <div class="presentation">
                                ...
                                <div id="Contenedor_Pedidos_Pendientes">
                                    <div>
                                        <div class="row" id="subcont_Imagen_Pedidos_Pendientes">
                                            <div class="col-sm-1"></div>
                                            <div class="col-sm-10"> <img src="../images/pedidosPendientes.svg" alt="Cover" id="imagenPedidosGeneral"> </div>
                                            <div class="col-sm-1"></div>
                                        </div>
                                        <div class="row" id="subcont_Texto_Pedidos_Pendientes">
                                            <div class="col-sm-1"></div>
                                            <div class="col-sm-10"> 
                                                          <p id="textoPedidoGeneral" class="animate__animated animate__backInLeft animate__delay">Pedidos Pendientes</p>
                                            </div>
                                            <div class="col-sm-1"></div>
                                            </div>
                                    </div>
                                </div><!-- FIN div id="Contenedor_Pedidos_Pendientes"-->

                            </div><!--FIN div class="presentation"-->




                        </div><!--FIN div class="anim__container container" v-if="statusSelected==1 && numstatus1==0"-->
                        ...
                        <div class="anim__container container" v-if="statusSelected==2 && numstatus2==0">
                            <div class="background">
                                ...
                                </svg><!--fin svg width="auto" height="-webkit-fill-available" viewBox="0 0 375 667" preserveAspectRatio="xMidYMid slice" fill="none" xmlns="http://www.w3.org/2000/svg"-->
                            </div><!-- fin div class="background"-->
                            ...
                            <div class="presentation">
                                ...
                                </svg><!--fin svg width="auto" height="75vh" viewBox="0 0 580 482" fill="none" xmlns="http://www.w3.org/2000/svg"-->
        
                                <div id="Contenedor_Pedidos_Enviados">
                                    <div>
                                        <div class="row" id="subcont_Imagen_Pedidos_Enviados">
                                            <div class="col-sm-1"></div>
                                                <div class="col-sm-10"> <img src="../images/pedidosEnviados.svg" id="imagenPedidosGeneral"> </div>
                                            <div class="col-sm-1"></div>
                                        </div>
                                        
                                        <div class="row" id="subcont_Texto_Pedidos_Enviados">
                                            <div class="col-sm-1"></div>
                                            <div class="col-sm-10">
                                                        <p id="textoPedidoGeneral" class="animate__animated animate__backInLeft animate__delay">Pedidos Enviados</p>
                                            </div>
                                            <div class="col-sm-1"></div>
                                        </div>
        
                                    </div>
                                </div><!--fin div id="Contenedor_Pedidos_Enviados"-->
                            </div><!--fin div class="presentation"-->
                        </div><!-- fin div class="anim__container container" v-if="statusSelected==2 && numstatus2==0"-->
                        ...
                        <div class="anim__container container" v-if="statusSelected==3 && numstatus3==0">
                            ...
                            <div class="presentation">
                                ...
                                <div id="Contenedor_Pedidos_Entregados">
                                    <div>
                                        <div class="row" id="subcont_Imagen_Pedidos_Entregados">
                                            <div class="col-sm-1"></div>
                                                <div class="col-sm-10"> <img src="../images/pedidosEntregados.svg" id="imagenPedidosGeneral"> </div>
                                            <div class="col-sm-1"></div>
                                        </div>

                                        <div class="row" id="subcont_Texto_Pedidos_Entregados">
                                            <div class="col-sm-1"></div>
                                            <div class="col-sm-10">
                                                <p id="textoPedidoGeneral" class="animate__animated animate__backInLeft animate__delay">Pedidos Entregados</p>
                                            </div>
                                            <div class="col-sm-1"></div>
                                        </div>
                                ...
                                </div><!-- fin div id="Contenedor_Pedidos_Entregados"-->
                            </div><!-- fin div class="presentation"-->
                        </div><!-- fin div class="anim__container container" v-if="statusSelected==3 && numstatus3==0"-->
                        ...
                        <div class="anim__container container" v-if="statusSelected==4 && numstatus4==0">
                        <div class="background">
                            <svg width="auto" height="-webkit-fill-available" viewBox="0 0 375 667" preserveAspectRatio="xMidYMid slice" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <g clip-path="url(#clip0)">
                                    <rect width="375" height="667" fill="white"/>
                        ...
                        <div class="presentation">
                            ...
                            <div id="Contenedor_Pedidos_Cancelados">
                                <div>
                                    <div class="row" id="subcont_Imagen_Pedidos_Cancelados">
                                        <div class="col-sm-1"></div>
                                            <div class="col-sm-10"> <img src="../images/pedidosCancelados.svg" id="imagenPedidosGeneral"> </div>
                                        <div class="col-sm-1"></div>
                                    </div>

                                    <div class="row" id="subcont_Texto_Pedidos_Cancelados">
                                        <div class="col-sm-1"></div>
                                        <div class="col-sm-10">
                                            <p id="textoPedidoGeneral" class="animate__animated animate__backInLeft animate__delay">Pedidos Cancelados</p>
                                        </div>
                                        <div class="col-sm-1"></div>
                                    </div>
                    ...
                    </div><!-- fin div class="anim__container container" v-if="statusSelected==4 && numstatus4==0"-->

                    


                    </div>
                </div><!-- FIN div v-if="pedidos.length" class="row"-->

                
                

            </div><!-- FIN <div class="container">-->
        </div><!-- FIN div v-if="isActive && initialize!=null" class="pedidos-container"-->
